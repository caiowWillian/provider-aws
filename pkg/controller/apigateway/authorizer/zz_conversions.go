...
func GenerateAuthorizer(resp *svcsdk.Authorizer) *svcapitypes.Authorizer {
	...
	if resp.ProviderARNs != nil {
		...
		cr.Spec.ForProvider.ProviderARNs = f8
	} else {
		cr.Spec.ForProvider.ProviderARNs = nil
	}
	if resp.Type != nil {
...
func GenerateCreateAuthorizerInput(cr *svcapitypes.Authorizer) *svcsdk.CreateAuthorizerInput {
	...
	if cr.Spec.ForProvider.ProviderARNs != nil {
		f7 := []*string{}
		for _, f7iter := range cr.Spec.ForProvider.ProviderARNs {
			var f7elem string
			f7elem = *f7iter
			f7 = append(f7, &f7elem)
		}
		res.SetProviderARNs(f7)
	}
	if cr.Spec.ForProvider.RestAPIID != nil {
		res.SetRestApiId(*cr.Spec.ForProvider.RestAPIID)
	}
